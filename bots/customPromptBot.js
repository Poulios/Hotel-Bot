...
const list1 = ['yes', 'y', 'yeap', 'yeah','ok', 'correct', 'agree', 'continue', 'sure', 'absolutely', 'alright', 'all right', 'definetely', 'certainly', 'agreed', 'of course']
const list2= ['no', 'n', 'nope', 'not at all', 'disagree', 'no way', 'not', 'negative', 'not much']
let reaskvar = false;
let availRooms = [];
let a;
...
let answersome='';
let answer = '';
const roomsNames = ['Executive Double Room', 'Deluxe Single Room', 'Deluxe Double Room', 'Superior Deluxe Double Room', 'Junior Suite Unter den Linden', 'Adlon Executive Suite', 'Junior Suite'];
...
class CustomPromptBot extends ActivityHandler {
    constructor(conversationState, userState) {
        ...
        this.onMessage(async (turnContext, next) => {
            ...
            if (Object.keys(profile1).length){
                ...
                const flow = await this.conversationFlow.get(turnContext, { lastQuestionAsked: question.startdate });
        ...
        switch (input.toLowerCase()) {
            ...
            default:{
                switch (flow.lastQuestionAsked) {
                    ...
                    // Ask the user for category and update the conversation flag.
                    case question.none:                        
                        if (profile.startdate && profiletemp.startdate){
                            result = this.validateDate(profile.startdate );
                            if (result.success) {
                                await turnContext.sendActivity(`I have your check in date as ${ result.date }.`);
                                await turnContext.sendActivity('Do you agree?'); 
                            } 
                        } else {
                            await turnContext.sendActivity("Let's get started. What date do you desire check in?"); 
                        }                        
                        dispatchvar3=false;
                        module.exports.dispatchvar3 = { dispatchvar3: dispatchvar3} 
                        ...
                        module.exports.dispatchvar4 = { dispatchvar4: dispatchvar4}
                        break;
                    // If we last asked for their check in date, record their response, confirm that we got it.
                    // Ask them for their check out date and update the conversation flag.
                    case question.startdate:
                        if (list1.includes(input.toLowerCase())){
                            if(!profile.finishdate||(!profiletemp.finishdate)){
                                await turnContext.sendActivity('What date do you desire check out?');                                 
                            } else if (profiletemp.finishdate){
                                result = this.validateDate(profile.finishdate );
                                if (result.success) {
                                    profile.finishdate = result.date; 
                                    await turnContext.sendActivity(`I have your check out date as ${ result.date }.`);                                
                                    await turnContext.sendActivity('Do you agree?'); 
                                }                               
                            }
                            ...
                            flow.lastQuestionAsked = question.startdate;
                            await turnContext.sendActivity('Give again when is your check-in date ?');
                            reaskvar = true;
                            module.exports.reaskvar = { reaskvar: reaskvar} 
                            break;
                        } else {                            
                            ...
                            if (result.success) {
                                ...
                                await turnContext.sendActivity(`I have your check in date as ${ profile.startdate }.`);
                                await turnContext.sendActivity('Do you agree?');
                                flow.lastQuestionAsked = question.startdate;
                                reaskvar = true;
                                module.exports.reaskvar = { reaskvar: reaskvar}    
                                ...
                                await turnContext.sendActivity("Please, provide check in date.");
                                break;
                            }
                        }  
                    // If we last asked for their category, record their response, confirm that we got it.
                        ...
                        if (list1.includes(input.toLowerCase())){           
                            ...
                            for (let i=0;i<availRooms.length;i+=2){
                                if (availRooms[i+1]>=clearnumber){
                                    ...
                                    if (availRooms[i].toLowerCase()==question.category.toLocaleLowerCase()){
                                        break;
                                    } else {
                            ...
                            if (desiredavailability!=''){
                                ...
                                answerdesired = 'If you want all the rooms to be exact the same, you can select among ' + desiredavailability + '.'; 
                            }
                            if (someavailability!=''){
                                someavailability = someavailability.slice(0,-2);     
                                if (desiredavailability!=''){
                                    answersome = 'Or you can select a combination of ' +someavailability; 
                                } else {
                                    answersome = 'However, you can select a combination of ' +someavailability; 
                                }                              
                            }
                            if (profile.category=='None'){
                                ...
                                if (someavailability!=''){                                       
                                    if (desiredavailability!=''){
                                        ...
                                        answersome = `Unfortunately, we do not have ${ profile.quantity } rooms of the same category. You can combine between ` +someavailability; 
                                    }
                                    await turnContext.sendActivity(answersome);                               
                                }
                                if (desiredavailability=='' && someavailability==''){
                                    await turnContext.sendActivity('We are sorry, but we do not have any others rooms available, that period.');
                                    await turnContext.sendActivity('Try again, with a different period.');
                                }
                            } else {                                
                                for (let i=0;i<availRooms.length;i+=2){
                                    if(availRooms[i].toLowerCase()==profile.category.toLowerCase()){
                                        if (availRooms[i+1]>=clearnumber){
                                            answer = 'Great, we have ' + profile.quantity + ' ' + availRooms[i] + ' from '+  profile.startdate + ' to '+ profile.finishdate + '. 🤗 ';
                                            await turnContext.sendActivity(answer);  
                                            break;
                                        } else if (availRooms[i+1]>=1){
                                            answer ='We have ' + availRooms[i+1] +' ' + profile.category + '.' ;
                                            await turnContext.sendActivity(answer);  
                                            break;
                                        } 
                                    }
                                ...
                                if (answer.slice(0,5)=='Great'){
                                        ...
                                        if (someavailability!=''){
                                            answersome = 'For the rest of your rooms, you can select a combination of '+ someavailability +'.';
                                            await turnContext.sendActivity(answersome);
                                        }     
                                        if (desiredavailability==''&& someavailability==''){
                                            await turnContext.sendActivity('We are sorry, but we do not have any others rooms available, that period.');
                                    ...
                                    answer ='We do not have any ' + profile.category + ' from '+  profile.startdate + ' to ' + profile.finishdate  + '. 😟';
                                    await turnContext.sendActivity(answer);  
                                    if (desiredavailability!='') {
                                        await turnContext.sendActivity(answerdesired);
                                    }
                                    if (someavailability!=''){
                                        answersome = 'Also, you can select a combination of '+ someavailability +'.';
                                        await turnContext.sendActivity(answersome);
                                    }   
                                    if (desiredavailability==''&& someavailability==''){
                                        await turnContext.sendActivity('We are sorry, but we do not have any rooms available that period.');
                                        await turnContext.sendActivity('Try again, with a different period.');
                                    }
                                }
                            }
                            ...
                            await turnContext.sendActivity('Give me again which room category do you desire?');
                            reaskvar = true;
                            module.exports.reaskvar = { reaskvar: reaskvar} 
                            break;
                        } else {
                            ...
                            if (result.success) {
                                ...
                                await turnContext.sendActivity(`I have your room category as ${ profile.category }.`);
                                await turnContext.sendActivity('Do you agree?');
                                flow.lastQuestionAsked = question.category;
                                reaskvar = true;
                                module.exports.reaskvar = { reaskvar: reaskvar}    
                                ...
                                await turnContext.sendActivity('I understand that you do not have a preference.');
                                ...
                                // If we couldn't interpret their input, ask them for it again.
                                // Don't update the conversation flag, so that we repeat this step.                                
                                await turnContext.sendActivity('What room category do you desire?');                                
                                break;
                            }
                        } 
                    // If we last asked for a check out date, record their response, confirm that we got it.
                    case question.finishdate:
                        if (list1.includes(input.toLowerCase())){                            
                                ...
                                if (!profiletemp.quantity){
                                    ...
                                    await turnContext.sendActivity(`I have  that you want ${profile.quantity} rooms. Do you agree?`);
                                }        
                                const { Connection, Request } = require("tedious");
                                ...
                                const config = {
                                    authentication: {
                                    options: {
                                        userName: "YOUR_SERVER_NAME", // update me
                                        password: "YOUR_PASSWORD" // update me
                                    },
                                    type: "default"
                                    },
                                    server: "YOUR_SQL_SERVER_NAME.database.windows.net", // update me
                                    options: {
                                    database: "YOUR_DB_NAME", //update me
                                    encrypt: true,
                                    validateBulkLoadParameters: true
                                    }
                                };
                                const connection = new Connection(config);
                                let stringquery = "SELECT rooms.type_id, COUNT(rooms.id) FROM rooms_type, rooms WHERE rooms_type.id = rooms.type_id AND rooms.id IN (SELECT id FROM rooms WHERE id NOT IN(SELECT rooms_id FROM rooms_booked AS rb WHERE bookings_id IN (SELECT id FROM bookings AS b WHERE (b.check_in_date <= " + "'" +profile.startdate + "'" + " AND b.check_out_date > "+ "'" + profile.finishdate +"'" + ") OR (b.check_in_date < " + "'" + profile.finishdate +"'" + "AND b.check_out_date >= " + "'" + profile.finishdate + "'" +") OR (" + "'" + profile.startdate +"'" + "<= b.check_in_date AND " + "'" + profile.finishdate + "'" + "> b.check_in_date)))) GROUP BY rooms.type_id";
                            ...
                            flow.lastQuestionAsked = question.finishdate;
                            await turnContext.sendActivity('Give again when is your check-out date?');
                            reaskvar = true;
                            module.exports.reaskvar = { reaskvar: reaskvar} 
                            break;
                        } else {     
                            ...
                            if (result.success) {
                                ...
                                if (f2< s2){
                                    console.log(f< s);
                                    await turnContext.sendActivity(`I have your check out date as ${ f}. Check out is earlier than check in (${ profile.startdate}).`);
                                    await turnContext.sendActivity('Give new check out date?');
                                    console.log(f<s);
                                } else {
                                    await turnContext.sendActivity(`I have your check out date as ${ f }.`);
                                    await turnContext.sendActivity('Do you agree?');
                                }       
                                console.log(f<s);                                 
                                flow.lastQuestionAsked = question.finishdate;
                                ...
                                await turnContext.sendActivity("Can you give me your check out date, please?");
                                break;
                            }
                        }  
                    case question.quantity:
                        if (list1.includes(input.toLowerCase())){   
                            ...
                            for (let i=0;i<availRooms.length;){
                                a= 'We have '+ availRooms[i+1] +' '+ availRooms[i] + ' available.'
                                console.log(a)
                                availRooms2.push(a)
                                i+=2;
                            }
                            ...
                            if (!profiletemp.category){
                                ...
                                await turnContext.sendActivity(`I have that you want ${ profile.category}. Do you agree?`);
                            }
                            reaskvar = false;
                            module.exports.reaskvar = { reaskvar: reaskvar} 
                            break;
                            ...
                            if (clearnumber) {
                                ...
                                await turnContext.sendActivity(`I have your number of rooms: ${ profile.quantity }.`);
                                await turnContext.sendActivity('Do you agree?');
                                flow.lastQuestionAsked = question.quantity;
                                reaskvar = true;
                                module.exports.reaskvar = { reaskvar: reaskvar}    
    ...
    // Validates category input. Returns whether validation succeeded and either the parsed and normalized
    ...
    static validateCategory(input) {
        ...
        for (let i=0;i<roomsNamesL.length;i++){
            if (category.toLowerCase()==roomsNamesL[i]){
                ...
                answerreturned = { success: false, message: 'Please enter a valid category.' };
            }
        }
        return answerreturned;
    }; 
    // Validates quantity input. Returns whether validation succeeded and either the parsed and normalized
    // value or a message the bot can use to ask the user again.
    static async validateQuantity(input) {
        ...
        if (clearnumber!='' && clearnumber>=1){
            ...
            answerreturned = { success: false, message: 'Please enter a valid number of rooms.' };
        }
        return answerreturned;
    }; 
            ...
            results.forEach(result => {
                ...
                if (results[0].resolution.values.length>1 ){
                    // The processed input contains a "value" entry if it is a date-time value, or "start" and
                        ...
                        const datetime = results[0].resolution.values[1].type === 'time'
                            ? new Date(`${ now.toLocaleDateString() } ${ datevalue }`)
                            : new Date(datevalue);
                        if (datetime && earliest < datetime.getTime()) {
                            output = { success: true, date: datetime.toLocaleDateString('en-US',{month:'long', year:'numeric', day:'numeric'}), date2: datetime};
                            return;
                        } else {
                            ...
                            const datetime2 = resolution.type === 'time'
                            ? new Date(`${ now.toLocaleDateString() } ${ datevalue2 }`)
                            : new Date(datevalue2);
                            if (datetime2 && earliest < datetime2.getTime()) {
                                output = { success: true, date: datetime2.toLocaleDateString('en-US',{month:'long', year:'numeric', day:'numeric'})};
                                return;
                            }
                        }
                } else {
                    result.resolution.values.forEach(resolution => {
                        ...
                        const datetime = resolution.type === 'time'
                            ...
                            : new Date(datevalue);
                        if (datetime && earliest < datetime.getTime()) {
                            output = { success: true, date: datetime.toLocaleDateString('en-US',{month:'long', year:'numeric', day:'numeric'})};
                            return;
                        } else if (datetime && (earliest - datetime.getTime()< (23*60*60*1000))){
                            output = { success: true, date: datetime.toLocaleDateString('en-US',{month:'long', year:'numeric', day:'numeric'})};
                            return;
                        } else {
                            const date1 = datevalue;
